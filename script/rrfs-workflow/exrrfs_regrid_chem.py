...
class RaveToMpasRegridProcessor:
    ...
    def initialize(self) -> None:
        ...
        if self.context.level_out_size > 1 and self.context.time_size > 1:
            print(
                "JLS, creating destination field with multiple levels and multiple times"
            )
    ...
    def run(self) -> None:
        ...
        if self.context.rank == 0:
            with open_nc(
                self.context.new_dst_path, mode="w", clobber=True, parallel=False
            ) as dst_nc:
                dst_nc.createDimension("nCells", self.context.num_cells)
                dst_nc.createDimension(
                    self.context.level_out_name, self.context.level_out_size
                )
                dst_nc.createDimension("StrLen", 64)
                if self.context.time_size > 1:
                    print(
                        "creating time dimension with size = "
                        + str(self.context.time_size)
                    )
    ...
    def create_src_field_wrapper(self, field_name: str) -> FieldWrapper:
        ...
        if field_name in ("PM25", "TPM"):
            ...
            src_data[:] = np.where(
                src_data < 0.0,
                0.0,
                src_data * 1.0e3 / area_data[:, :, np.newaxis] / 3600.0,
            )
...
def main() -> None:
    ...
    if testpath.exists():
        ...
        scrip_path = Path(
            workdir + "/mpas_" + dataset_name + "-" + mesh_name + "_scrip.nc"
        )
    #
    ...
    if dataset_name == "RAVE":
        ...
        dates_needed = []
        for i in range(24):
            x = datetime(int(YYYY), int(MM), int(DD), int(HH), 0, 0) - timedelta(
                hours=i
            )
        ...
        field_names = (
            ...
            "PM10-PRI",
            "h_agl",
        )  # ,"HC01"=methane BAQMS, summer, 2025
    ...
    if dataset_name == "RAVE":
        for date_to_process in dates_needed:
            rave_paths = glob.glob(
                input_dir + "/RAVE-HrlyEmiss-3km_v2r0_blend_s" + date_to_process + "*"
            )
            if len(rave_paths) == 0:
                print(
            ...
            new_dst_path = Path(
                output_dir + "/" + mesh_name + "-RAVE-" + date_to_process + ".nc"
            )
        ...
        for date_to_process in dates_needed:
            print(
                "JLS, looking for file " + input_dir + "fmc_" + date_to_process + ".nc"
            )
            rave_paths = glob.glob(input_dir + "fmc_" + date_to_process + ".nc")
            rave_path = rave_paths[0]
            new_dst_path = Path(
                output_dir + "fmc_" + date_to_process + "_" + mesh_name + ".nc"
            )
    ...
    #
        ...
        rave_path = Path(
            input_dir + "/GRA2PESv1.0_total_2021" + MM + "_" + DOWs + "_00to11Z.nc"
        )
        new_dst_path = Path(
            output_dir + "/" + dataset_name + "v1.0_total_" + mesh_name + "_00to11Z.nc"
        )
        context = RaveToMpasRegridContext(
            dataset_name=dataset_name,
            src_path=rave_path,
        ...
        rave_path = Path(
            input_dir + "/GRA2PESv1.0_total_2021" + MM + "_" + DOWs + "_12to23Z.nc"
        )
        new_dst_path = Path(
            output_dir + "/" + dataset_name + "v1.0_total_" + mesh_name + "_12to23Z.nc"
        )
        context = RaveToMpasRegridContext(
            dataset_name=dataset_name,
            src_path=rave_path,
        ...
        if dataset_name == "PECM":
            rave_path = Path(
                input_dir + "/pollen_obs_" + YYYY + "_BELD6_ef_T_" + JJJ + ".nc"
            )
            new_dst_path = Path(
                output_dir + "/pollen_ef_" + mesh_name + "_" + YYYY + "_" + JJJ + ".nc"
            )
        elif dataset_name == "NEMO":
            rave_path = Path(input_dir + "/NEMO_RWC_POC_PEC_PMOTHR.annual.2017.nc")
            new_dst_path = Path(
                output_dir + "/NEMO_RWC_ANNUAL_TOTAL_" + mesh_name + ".nc"
            )
        elif dataset_name == "NARR":
            rave_path = Path(input_dir + "/rwc_emission_denominator.2017.nc")
            new_dst_path = Path(
                output_dir + "/NEMO_RWC_DENOMINATOR_2017_" + mesh_name + ".nc"
            )
        elif dataset_name == "ECOREGION":
            ...
            new_dst_path = Path(
                output_dir + "/FENGSHA_2022_NESDIS_inputs_" + mesh_name + "_v3.2.nc"
            )
        elif dataset_name == "FENGSHA_2":
            rave_path = Path(input_dir + "/LAI_GVF_PC_DRAG_CLIMATOLOGY_2024v1.0.nc4")
            new_dst_path = Path(
                output_dir
                + "/LAI_GVF_PC_DRAG_CLIMATOLOGY_2024v1.0."
                + mesh_name
                + ".nc"
            )
