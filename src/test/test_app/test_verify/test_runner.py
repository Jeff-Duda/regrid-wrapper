...
from regrid_wrapper.app.verify.context import VerifyContext, VerifyPair
from regrid_wrapper.app.verify.runner import NccmpError, run_verify
...
def test_netcdf_values_differ(verify_ctx: VerifyContext, tmp_path: Path, fail_fast: str | bool) -> None:
    ...
    data = verify_ctx.model_dump()
    new_verify_pair = VerifyPair(actual=Path(actual2_path.name), expected=verify_ctx.verify_pairs[0].expected)
    verify_pairs = list(data["verify_pairs"])
    verify_pairs.append(new_verify_pair)
    data["verify_pairs"] = verify_pairs
